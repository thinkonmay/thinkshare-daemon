...
#define SERVICE_NAME "ThinkremoteSvc"
...
CreateJobObjectForChildProcess() {
  ...
  // Kill daemon.exe when the final job object handle is closed (which will happen if we terminate unexpectedly).
  // This ensures we don't leave an orphaned daemon.exe running with an inherited handle to our log file.
  job_limit_info.BasicLimitInformation.LimitFlags |= JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE;

  // Allow daemon.exe to use CREATE_BREAKAWAY_FROM_JOB when spawning processes to ensure they can to live beyond
  // the lifetime of ThinkremoteSvc.exe. This avoids unexpected user data loss if we crash or are killed.
...
OpenLogFileHandle() {
  ...
  // Create thinkremote.log in the Temp folder (usually %SYSTEMROOT%\Temp)
  GetTempPathW(_countof(log_file_name), log_file_name);
  wcscat_s(log_file_name, L"thinkremote.log");
  ...
  // Overwrite the old thinkremote.log
...
RunTerminationHelper(HANDLE console_token, DWORD pid) {
  ...
  // This will allow us to attach to Thinkremote's console and send it a Ctrl-C event.
...
ServiceMain(DWORD dwArgc, LPTSTR *lpszArgv) {
  ...
  // Only allow daemon.exe to inherit the log file handle, not all inheritable handles
  ...
  // Loop every 3 seconds until the stop event is set or daemon.exe is running
  while (WaitForSingleObject(stop_event, 3000) != WAIT_OBJECT_0) {
    ...
    // Start daemon.exe inside our job object
    UpdateProcThreadAttribute(startup_info.lpAttributeList,
      0,
    ...
    // Wait for either the stop event to be set or daemon.exe to terminate
    ...
    switch (WaitForMultipleObjects(_countof(wait_objects), wait_objects, FALSE, INFINITE)) {
      case WAIT_OBJECT_0:
        // The service is shutting down, so try to gracefully terminate daemon.exe.
...
DoGracefulTermination(DWORD pid) {
  // Attach to Thinkremote's console
  ...
  // Send a Ctrl-C event to Thinkremote
...
main(int argc, char *argv[]) {
  ...
  // Check if this is a reinvocation of ourselves to send Ctrl-C to daemon.exe
  ...
  // We want to use the directory where daemon.exe is located instead of system32.
